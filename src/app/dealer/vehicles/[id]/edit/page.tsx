...
// ─────────────────────────────────────────────────────────────
// ✅ Fix: Remove required_error and invalid_type_error from the .number() chain
// These options don't exist at that level in Zod
const editVehicleSchema = z.object({
  ...
  status: z.enum(['DRAFT', 'PENDING_REVIEW', 'APPROVED', 'PUBLISHED', 'SOLD', 'ARCHIVED']).default('PUBLISHED'),
});
...
export default function EditVehiclePage() {
  ...
  const {
    ...
    // ✅ Use the schema with proper typing - use as unknown to avoid any warning
    resolver: zodResolver(editVehicleSchema) as unknown as any,
    defaultValues: {
      status: 'PUBLISHED',
      currency: 'NGN',
    },
    ...
    try {
      ...
      if (vehicle) {
        ...
        if (Array.isArray(vehicle.features)) {
          ...
          try {
            setSelectedFeatures(JSON.parse(vehicle.features));
          } catch {
            setSelectedFeatures([]);
          }
        }
        ...
        reset({
          ...
          status: (vehicle.status as VehicleFormData['status']) || 'PUBLISHED',
        });
      } else {
        showToast({
          type: 'error',
          title: 'Asset Not Found',
          message: 'Vehicle record does not exist.',
        });
        router.push('/dealer/inventory');
      }
    } catch (error) {
      console.error('[EditVehiclePage] Error loading vehicle:', error);
      showToast({
        type: 'error',
  ...
  }, [vehicleId, reset, router, showToast]);
  ...
  const onSubmit = async (data: VehicleFormData) => {
    ...
    try {
      await InventoryService.updateVehicle(vehicleId, {
        ...data,
        images,
      ...
      console.error('[EditVehiclePage] Update error:', error);
      showToast({
        type: 'error',
        title: 'Update Failed',
  ...
  const handleDelete = async () => {
    ...
    try {
      await InventoryService.deleteVehicle(vehicleId);
      ...
      console.error('[EditVehiclePage] Delete error:', error);
      showToast({
        type: 'error',
        title: 'Action Failed',
  ...
  const assetHeading = watchMake && watchModel ? `${watchYear || ''} ${watchMake} ${watchModel}` : 'Vehicle';
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-muted uppercase">ID: #{vehicleId.slice(0, 8)}</span>
            </div>
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6 bg-graphite/95 border-border/80 space-y-4">
              ...
              <Button
                ...
                loadingText="Saving..."
                className="mt-2 text-xs uppercase tracking-widest font-semibold flex items-center justify-center gap-2 h-12"
              >
